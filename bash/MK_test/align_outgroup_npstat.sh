...
if [ 1 == 2 ] ; then
    ...
    # do alignment
    echo "running ------- lastal "
    ${LAST_PATH}lastal -P8 -f MAF -m2 -D1e9 -C2 --split -p C_maculatus_${SPECIES}.train Cmacdb ${SPECIES}.fasta.masked | ${LAST_PATH}last-split -r | ${LAST_PATH}maf-linked - > out_${SPECIES}.maf
    echo "done ---------- lastal "
...
if [ 1 == 2] ; then
    ...
    ${LAST_PATH}maf-convert bed out_${SPECIES}.maf | sort -k1,1 -k2,2n > aln_${SPECIES}.sorted.bed
...
if [ 1 == 2] ; then
    MAFTOOLS_PATH=/proj/coleoptera-genomics-2025/snic2021-6-30/Milena/software_install/mafTools/mafTools/bin/
    ${MAFTOOLS_PATH}mafToFastaStitcher \
        --maf out_${SPECIES}.maf \
        --seqs C_maculatus.fasta.masked,${SPECIES}.fasta.masked \
        --breakpointPenalty 1 \
        --interstitialSequence 0 \
        --outMfa projected_aln_C_maculatus_${SPECIES}.mfa

fi

head -3 out_${SPECIES}.maf
wc -l out_${SPECIES}.maf
ls -la out_${SPECIES}.maf