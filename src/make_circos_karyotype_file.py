    ...
    with open(outfile_name, "w") as circos_outfile:
        ...
        for record in SeqIO.parse(assembly, "fasta"):
            if len(record.seq) > min_contig_length:
                ...
                if label in X_list:
                    entry = ["chr - "+record.id.split(":")[0], label, "0", str(len(record.seq)), colors["X"]] # split the record.id at ":" because that confuses circos
                else:
                    entry = ["chr - "+record.id.split(":")[0], label, "0", str(len(record.seq)), colors["A"]] # split the record.id at ":" because that confuses circos
                karyotype_entries.append(entry)
                circos_outfile.write(" ".join(entry)+"\n")
