...
\begin{figure}[H]
    ...
    \caption{\textbf{positively selected orthologs split into autosomal or X-linked.} Positively selected genes are determined via likelihood ratio test comparison between paml site models M1a and M2a. The within-family comparisons are performed for three species groups: \textit{Bruchini} (\textbf{A}), \textit{Coccinella} (\textbf{B}) and \textit{Tribolium} (\textbf{C}).
    }\label{site_models}
...
\begin{figure}[H]
    ...
    \begin{subfigure}[t]{0.49\linewidth}
	    ...
	    %\makebox[0pt][l]{\hspace{30pt}\textbf{C}}
	    \begin{picture}(0,0)
   	  		\put(30,-5){\textbf{a}} % adjust the x and y values here
		\end{picture}
	    \begin{center}
    ...
    \caption{\textbf{Differential expression analysis.} We are utilizing the log2 fold change (LFC) information from genes in \textit{C. maculatus} and combining it with d$_N$/d$_S$ between these genes and their 1-to-1 orthologs in other bruchids. \textbf{A}: number of sex biased genes in abdominal (reproductive) tissues and head and thorax (somatic) tissues. \textbf{B-D}: Scatterplot of significantly differentially expressed genes showing d$_N$/d$_S$ of \textit{A. obtectus} (\textbf{B}), \textit{B. siliquastri} (\textbf{C}) and \textit{C. chinensis} (\textbf{D}) respectively.
    }\label{DE_analysis}
